...
def find_films_by_actor(doca, out = [])
  begin	                                                              #throw exception if something wrong happen
	puts "Entering find_actors_in_film with #{find_name_title(doca)}."
 	next1=doca.xpath('//table[@class="wikitable sortable"]').first    #locate the film table
	tds = next1.xpath('tr').each do |tr|                              #go into tr node
	td=tr.xpath('td').each do |td|                                    #go into td node
   		 a=td.xpath('i/a').map { |link| link['href'] }                #go into i/<a href> node and pass the value to array 'out'
   	 	 out<<a
    	end
	end
 	out.uniq                                                          #return uniq array 'out'
  rescue
  	puts "It seems something go wrong...."
  end
end
